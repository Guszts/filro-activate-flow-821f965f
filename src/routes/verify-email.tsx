...
function VerifyEmailPage() {
    ...
    return (
      <main className="min-h-screen grid place-items-center px-5 py-10">
        <div className="text-center">
          <p className="text-ink-soft mb-4">E-mail não informado.</p>
  ...
  const verify = async (e: React.FormEvent) => {
    ...
    if (token.length !== 6) return toast.error("Digite os 6 dígitos do código");
    setLoading(true);
    const { data, error } = await supabase.auth.verifyOtp({ email, token, type: "email" });
    setLoading(false);
    if (error) return toast.error(error.message);
    if (!data.session) return toast.error("Não foi possível confirmar. Tente reenviar o código.");
  ...
  return (
    <main className="min-h-screen grid place-items-center px-5 py-10">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        ...
        <h1 className="mt-8 editorial-headline text-4xl sm:text-5xl text-ink">Confirme seu e-mail</h1>
        <p className="mt-3 text-ink-soft">
          Enviamos um código de 6 dígitos para <strong className="text-ink">{email}</strong>.
        </p>
